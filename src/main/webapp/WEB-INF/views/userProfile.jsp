      ...
      <table class="table table-dark">
        ...
        <c:forEach var="list" items="${addressModels}">
          ...
          <th scope="row">#</th>
            ...
            <td>${list.state}</td>
            <td>${list.zipCode}</td>
            <td><a href="addressEdit?id=${list.addressId}" class="badge badge-primary">Edit</a></td>
            <td><a href="delete?id=${list.addressId}&&userId=${user.userId}" class="badge badge-primary">Delete</a></td>
          </tr>
      </c:forEach>
  </thead>
</table>
<a href="newAddress?userId=${user.userId}" button type="button" class="btn btn-primary btn-lg btn-block">Add new Address</button></a>

<c:if test="${not empty user.accountModelList}">
<b>ACCOUNT INFO:</b>
      <table class="table table-dark">
        <thead>
          <tr>
